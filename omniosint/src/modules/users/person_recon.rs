...
impl OsintModule for PersonReconModule {
    fn name(&self) -> String {
        "Name Permutation Engine".to_string()
    }
    fn description(&self) -> String {
        "Gera usernames a partir de Nome Real".to_string()
    }

    async fn run(&self, target: &Target, _client: &reqwest::Client) -> Result<Vec<Target>> {
        if target.kind != TargetType::RealName {
            return Ok(vec![]);
        }
        ...
        for p in perms {
            findings.push(Target::new(&p, TargetType::Username));
        }
        Ok(findings)
    }
}
