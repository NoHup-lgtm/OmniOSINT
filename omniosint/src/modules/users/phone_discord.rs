...
impl OsintModule for PhoneDiscordModule {
    fn name(&self) -> String {
        "ID & Phone Intel".to_string()
    }
    fn description(&self) -> String {
        "Analisa metadados".to_string()
    }
    ...
    async fn run(&self, target: &Target, _client: &reqwest::Client) -> Result<Vec<Target>> {
        ...
        if target.kind == TargetType::Phone {
            ...
            if num.starts_with("+55") {
                findings.push(Target::new("Loc: Brasil", TargetType::Location));
            } else if num.starts_with("+1") {
                findings.push(Target::new("Loc: USA", TargetType::Location));
            }
        }
        ...
        if target.kind == TargetType::DiscordID {
            if let Ok(id) = target.value.parse::<u64>() {
                ...
                findings.push(Target::new(
                    &format!("Discord Created (Epoch): {}", timestamp),
