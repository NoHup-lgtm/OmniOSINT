...
impl OsintModule for SocialCheckerModule {
    fn name(&self) -> String { "Social Media Hunter".to_string() }
    fn description(&self) -> String { "Verifica redes sociais".to_string() }
    ...
    async fn run(&self, target: &Target, client: &reqwest::Client) -> Result<Vec<Target>> {
        ...
        for (_, tpl) in sites {
            ...
            if let Ok(resp) = client.get(&url).send().await {
                if resp.status().is_success() { findings.push(Target::new(&url, TargetType::Domain)); }
            }
        }
        Ok(findings)
    }
