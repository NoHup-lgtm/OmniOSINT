...
#[derive(Deserialize)]
struct GeoResponse {
    status: String, country: Option<String>, city: Option<String>, 
    lat: Option<f64>, lon: Option<f64>, isp: Option<String>,
}
...
impl OsintModule for GeoIpModule {
    fn name(&self) -> String { "GeoIP Location Tracker".to_string() }
    fn description(&self) -> String { "Rastreia a localização física e ISP".to_string() }

    async fn run(&self, target: &Target, client: &reqwest::Client) -> Result<Vec<Target>> {
        if target.kind != TargetType::IP { return Ok(vec![]); }
        let ip = &target.value;
        if ip.starts_with("192.168") || ip.starts_with("127.") || ip.starts_with("10.") { return Ok(vec![]); }
            ...
            if let Ok(geo) = resp.json::<GeoResponse>().await {
                if geo.status == "success" {
                    ...
                    if let (Some(c), Some(co)) = (geo.city, geo.country) {
                        findings.push(Target::new(&format!("Loc: {}, {}", c, co), TargetType::Location));
                    }
                    if let (Some(lat), Some(lon)) = (geo.lat, geo.lon) {
                        findings.push(Target::new(&format!("Coords: {}, {}", lat, lon), TargetType::Location));
                    }
